...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Use Web3Forms client-side redirect method (bypasses bot challenges)
      const web3formsUrl = `https://api.web3forms.com/redirect?access_key=0be3cced-4182-4e7b-a3ef-f837e78f24b5&from_name=${encodeURIComponent(name as string)}&email=${encodeURIComponent(email as string)}&subject=New%20Restaurant%20Submission%3A%20${encodeURIComponent(restaurant as string)}&message=${encodeURIComponent(`Name: ${name}\nEmail: ${email}\nRestaurant: ${restaurant}\nMessage: ${message}`)}&redirect=${encodeURIComponent(`${window.location.origin}/thanks`)}`;
